...
def find_header_and_data(df):
    ...
    if max_matches > 0:
        headers = [normalize_text_std(val) for val in df.iloc[best_row_idx].values]
        df_data = df.iloc[best_row_idx + 1:].reset_index(drop=True)
        return df_data, headers, best_row_idx
    
    headers = [normalize_text_std(col) for col in df.columns]
    return df, headers, -1
